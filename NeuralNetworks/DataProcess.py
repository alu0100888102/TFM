...
def loaddata_split_crossval(route, fro, to):
    d = []
    l = []
    for e in range(fro-1, to):
        filename1 = "bad" + str(e+1) + ".txt"
    ...
    for i in range(nlogs):
        ...
        for k in d[i]["PL"]:
            temporal.append(k)
        npdata.append(np.array(temporal, dtype=np.float32))
        if l[i] == 0:
            nplabels.append(0)
        else:
            nplabels.append(1)
...
def loaddata_split_wholefile_tree(route, fro, to):
    npdata = []
    nplabels = []
    for e in range(fro - 1, to):
        d = []
    ...
    out = [npdata, nplabels]
    return out
def loaddata_split_wholefile(route, fro, to):
    npdata = []
    nplabels = []
    for e in range(fro - 1, to):
        d = []
        ...
        npdata.append(np.array(rerex, dtype=np.float32))
        nplabels.append(filename1)
...
def loaddata_split_tree(route, fro, to):
    d = []
    l = []
    for e in range(fro-1, to):
        filename1 = "bad" + str(e+1) + ".txt"
...
def loaddata_split_LSTM_wholefile(route, fro, to, size):
    npdata = []
    nplabels = []
    size = size-1
    for e in range(fro - 1, to):
...
def loaddata_split_LSTM_moving_crossval(route, fro, to, size):
    npdata = []
    nplabels = []
    for e in range(fro-1, to):
        d = []
